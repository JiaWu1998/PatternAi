...
const startGuessNudgeDirection = () => {
    for(let c=0; c<weightMapLayers[1].length; c++){
        let arrayHolder = [];
        for(let b=0; b<weightMapLayers[1][c].length; b++){
            let dir = (Math.random() < 0.5) ? -1 : 1;
            arrayHolder.push(dir);
        }
        nudgeDirection[1].push(arrayHolder);
    }
    for(let v=0; v<weightMapLayers[0].length; v++){
        let arrayHolder = [];
        for(let m=0; m<weightMapLayers[0][v].length; m++){
            let dir = (Math.random() < 0.5) ? -1 : 1;
            arrayHolder.push(dir);
        }
        nudgeDirection[0].push(arrayHolder);
    }
};
...
const backProp = (thisIterationAgain) => {
    ...
    for(let j=0; j<nudgeDirection.length; j++){
        for(let m=0; m<nudgeDirection[j].length; m++){
            for(let v=0; v<nudgeDirection[j][m].length; v++){
                let current = activationLayers[2]
                nudgeDirection[j][m][v] = -nudgeDirection[j][m][v];
                let costIdx = (j == 1) ? 0 : 1;
                if((weightMapLayers[j][m][v] + (nudgeDirection[j][m][v]*costs[costIdx]) <= 10) && (weightMapLayers[j][m][v] + (nudgeDirection[j][m][v]*costs[costIdx]) >= -10)){
                    weightMapLayers[j][m][v] = weightMapLayers[j][m][v] + (nudgeDirection[j][m][v]*costs[costIdx]);
                }
                eraseActivations();
                ...
                if((differenceCurrentToCorrect[0]+differenceCurrentToCorrect[1]) < (differenceCalculatedToCorrect[0]+differenceCalculatedToCorrect[1])){
                    nudgeDirection[j][m][v] = -nudgeDirection[j][m][v];
                    weightMapLayers = weightMapCopy.slice(0);
                    eraseActivations();
                    forwardProp(thisIterationAgain);
                }
...
// console.log("acttivationMap");
// console.log(activationLayers);
// console.log("nudgeMap");
